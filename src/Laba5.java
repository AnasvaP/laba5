...
public class Laba5 {
    ...
    public static void main(String[] args) {
        long mmm = System.currentTimeMillis();
            ...
            int x1Min = -2, x2Min = -9, x3Min = -5, x1Max = 7, x2Max = 2, x3Max = 1,
                    xAverageMax = (x1Max + x2Max + x3Max) / 3, xAverageMin = (x1Min + x2Min + x3Min) / 3,
                    yMax = 200 + xAverageMax, yMin = 200 + xAverageMin, N = 15, m = 3, f1 = m - 1, f2 = N, f3 = f1 * f2, n = 15;
            ...
            double[][] matrixForX = {
                    ...
                    {7, 2, -5, 0, 0, 0, 0, 0, 0, 0},
                    {7, 2, 1, 0, 0, 0, 0, 0, 0, 0},
                    {-2.9675, -3.5, -2, 0, 0, 0, 0, 0, 0, 0},
                    {2.9675, -3.5, -2, 0, 0, 0, 0, 0, 0, 0},
                    {2.5, -10.1825, -2, 0, 0, 0, 0, 0, 0, 0},
                    {2.5, 3.1825, -2, 0, 0, 0, 0, 0, 0, 0},
            ...
            for (int j = 0; j < 3; j++) {
                ...
                averageY9 += matrixForY[8][j] / 3;
                averageY10 += matrixForY[9][j] / 3;
                averageY11 += matrixForY[10][j] / 3;
                averageY12 += matrixForY[11][j] / 3;
                averageY13 += matrixForY[12][j] / 3;
                averageY14 += matrixForY[13][j] / 3;
            ...
            BigDecimal koefB11 = determinantCalcb11.determinant().divide(determinantCalcDet.determinant(), 8, RoundingMode.HALF_EVEN);
            BigDecimal koefB22 = determinantCalcb22.determinant().divide(determinantCalcDet.determinant(), 8, RoundingMode.HALF_EVEN);
            BigDecimal koefB33 = determinantCalcb33.determinant().divide(determinantCalcDet.determinant(), 8, RoundingMode.HALF_EVEN);
            ...
            for (int i = 0; i < 15; i++) {
                test = koefB0.doubleValue()
                        ...
                        + koefB11.doubleValue() * Math.pow(matrixForX[i][0], 2)
                        + koefB22.doubleValue() * Math.pow(matrixForX[i][1], 2)
                        + koefB33.doubleValue() * Math.pow(matrixForX[i][2], 2);
            ...
            double dispersion1 = 0, dispersion2 = 0, dispersion3 = 0, dispersion4 = 0,
                    ...
                    dispersion13 = 0, dispersion14 = 0, dispersion15 = 0;
            for (int j = 0; j < 3; j++) {
                ...
                dispersion15 += (Math.pow((matrixForY[14][j] - averageY15), 2)) / 3;
            }

            double[] dispersionArray = {dispersion1, dispersion2, dispersion3, dispersion4, dispersion5, dispersion6, dispersion7,
                    dispersion8, dispersion9, dispersion10, dispersion11, dispersion12, dispersion13, dispersion14, dispersion15};
            System.out.println();
            ...
            double maxDispersion1 = Math.max(Math.max(Math.max(dispersion1, dispersion2), Math.max(dispersion3, dispersion4)),
                    ...
                    maxDispersion2 = Math.max(Math.max(Math.max(dispersion9, dispersion10), Math.max(dispersion11, dispersion12)),
                            Math.max(Math.max(dispersion13, dispersion14), Math.max(dispersion15, 0))), maxDispersion = Math.max(maxDispersion1, maxDispersion2);
            ...
            if (gp > CohrenaTable[f2 - 1][f1 - 1]) {
                ...
                numberOfY += 1;
                int[][] matrixForYNew = new int[15][numberOfY];
                ...
                for (int j = 0; j < numberOfY; j++) {
                    ...
                    averageY15 += matrixForYNew[14][j] / numberOfY;
                }
                ...
                double dispersionNew1 = 0, dispersionNew2 = 0, dispersionNew3 = 0, dispersionNew4 = 0,
                        ...
                        dispersionNew13 = 0, dispersionNew14 = 0, dispersionNew15 = 0, sumOfDispersionNew = 0;
                for (int j = 0; j < numberOfY; j++) {
                    ...
                    dispersionNew15 += (Math.pow((matrixForYNew[14][j] - averageY15), 2)) / 4;
                }
                ...
                double[] newDispersionArray = {dispersionNew1, dispersionNew2, dispersionNew3, dispersionNew4, dispersionNew5, dispersionNew6, dispersionNew7,
                        dispersionNew8, dispersionNew9, dispersionNew10, dispersionNew11, dispersionNew12, dispersionNew13, dispersionNew14, dispersionNew15};
                for (int i = 0; i < 15; i++) {
                ...
                double maxDispersionNew1 = Math.max(Math.max(Math.max(dispersionNew1, dispersionNew2), Math.max(dispersionNew3, dispersionNew4)),
                        ...
                        maxDispersionNew2 = Math.max(Math.max(Math.max(dispersionNew9, dispersionNew10), Math.max(dispersionNew11, dispersionNew12)),
                                Math.max(Math.max(dispersionNew13, dispersionNew14), Math.max(dispersionNew15, 0))), maxDispersionNew = Math.max(maxDispersionNew1, maxDispersionNew2);
            ...
            int d = 0;
            // boolean b0Boolean , b1Boolean , b2Boolean ,b3Boolean ,b12Boolean ,b13Boolean ,b23Boolean ,b123Boolean, b11Boolean,b22Boolean,b33Boolean ;
